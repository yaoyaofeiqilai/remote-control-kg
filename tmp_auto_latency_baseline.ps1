...
$sampleFile = "auto_samples_audio_baseline_$ts.jsonl"
$outLog = "tmp_latency_audio_baseline_$ts.out.log"
$errLog = "tmp_latency_audio_baseline_$ts.err.log"
...
for($i=0;$i -lt 100;$i++){
  ...
  try {
    ...
    $ah = Invoke-RestMethod -Uri "$base/api/audio_health" -TimeoutSec 1
    ...
    $obj = [ordered]@{
      ...
      audio_capture_running = [bool]$ah.capture_running
      audio_client_up = [bool]$ah.client_up
      audio_last_error = [string]$ah.last_error
      audio_muted = [bool]$ah.system_mute.muted
    }
...
$records = $rows | ForEach-Object { try { $_ | ConvertFrom-Json } catch { $null } } | Where-Object { $_ -and $_.playout_delay_ms -ne $null -and [double]$_.playout_delay_ms -gt 0 }
$connected = $rows | ForEach-Object { try { $_ | ConvertFrom-Json } catch { $null } } | Where-Object { $_ -and $_.client_up -eq $true }
$audioOn = $rows | ForEach-Object { try { $_ | ConvertFrom-Json } catch { $null } } | Where-Object { $_ -and $_.audio_capture_running -eq $true }
if($records.Count -gt 0){
  $vals = $records | ForEach-Object { [double]$_.playout_delay_ms }
  ...
  Write-Output "SAMPLES_WITH_DELAY=$($vals.Count)"
  Write-Output "CLIENT_UP_COUNT=$($connected.Count)"
  Write-Output "AUDIO_CAPTURE_RUNNING_COUNT=$($audioOn.Count)"
  ...
  Write-Output "SAMPLE_FILE=$sampleFile"
  Write-Output "NO_VALID_DELAY_SAMPLES"
  Write-Output "CLIENT_UP_COUNT=$($connected.Count)"
  Write-Output "AUDIO_CAPTURE_RUNNING_COUNT=$($audioOn.Count)"
}
