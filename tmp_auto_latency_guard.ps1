...
$sampleFile = "auto_samples_audio_guard_$ts.jsonl"
$outLog = "tmp_latency_audio_guard_$ts.out.log"
$errLog = "tmp_latency_audio_guard_$ts.err.log"
...
if($records.Count -gt 0){
  ...
  $tail = @()
  if($vals.Count -gt 20){ $tail = $vals[($vals.Count-20)..($vals.Count-1)] } else { $tail = $vals }
